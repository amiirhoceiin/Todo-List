...
export default function Todoinput({addone}: Props) {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>)=> {
    ...
    if(title && description){
      addone({title, description, complected: false});
      setTitle('');
      setDescription('');
    }
    else {
  ...
  return (
    <form onSubmit={handleSubmit} className='w-full max-w-4xl mx-auto bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4'>
      <input className='shadow appearance-none border rounded w-full my-2 py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline' type='text' placeholder='title' value={title} onChange={(e : React.ChangeEvent<HTMLInputElement>)=>setTitle}/>
